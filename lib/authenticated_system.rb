module AuthenticatedSystem
  protected
    ...
    def self.included(base)
      base.send :helper_method, :current_author, :logged_in?, :authorized? if base.respond_to? :helper_method
    end
    ...
    # for the paranoid: we _should_ be storing author_token = hash(cookie_token, request IP)
    ...
    def logout_keeping_session!
      ...
      @current_author.forget_me if @current_author.is_a? Author
      @current_author = false     # not logged in, and don't do it for me
      kill_remember_cookie!     # Kill client-side auth cookie
      session[:author_id] = nil   # keeps the session but kill our variable
    ...
    def valid_remember_cookie?
      return nil unless @current_author
      (@current_author.remember_token?) && 
        (cookies[:auth_token] == @current_author.remember_token)
    end
    ...
    def handle_remember_cookie!(new_cookie_flag)
      ...
      case
      when valid_remember_cookie? then @current_author.refresh_token # keeping same expiry date
      when new_cookie_flag        then @current_author.remember_me 
      else                             @current_author.forget_me
      end
    ...
    def send_remember_cookie!
      cookies[:auth_token] = {
        :value   => @current_author.remember_token,
        :expires => @current_author.remember_token_expires_at }
    end
