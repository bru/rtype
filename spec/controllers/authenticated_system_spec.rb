...
describe SessionsController do
  ...
  describe "logout_killing_session!" do
    ...
    it 'nils the current author'      do logout_killing_session!; current_author.should be_nil end
    it 'kills :author_id session' do
      session.stub!(:[]=)
      session.should_receive(:[]=).with(:author_id, nil).at_least(:once)
      logout_killing_session!
    end
    it 'forgets me' do    
      current_author.remember_me
      current_author.remember_token.should_not be_nil; current_author.remember_token_expires_at.should_not be_nil
      Author.find(1).remember_token.should_not be_nil; Author.find(1).remember_token_expires_at.should_not be_nil
      logout_killing_session!
      Author.find(1).remember_token.should     be_nil; Author.find(1).remember_token_expires_at.should     be_nil
    end
  end
  ...
  describe "logout_keeping_session!" do
    ...
    it 'nils the current author'      do logout_keeping_session!; current_author.should be_nil end
    it 'kills :author_id session' do
      session.stub!(:[]=)
      session.should_receive(:[]=).with(:author_id, nil).at_least(:once)
  ...
  describe "Logging in by cookie" do
    def set_remember_token token, time
      @author[:remember_token]            = token; 
      @author[:remember_token_expires_at] = time
      @author.save!
    end    
    before do 
