...
describe SessionsController do
  ...
  before do 
    ...
    Author.stub!(:authenticate).with(@login_params[:login], @login_params[:password]).and_return(@author)
  end
  def do_create
        ...
        describe "my request cookie token is #{has_request_token.to_s}," do
          describe "and ask #{want_remember_me ? 'to' : 'not to'} be remembered" do 
            before do
              ...
              @ccookies.stub!(:[]=)
              @author.stub!(:remember_me) 
              @author.stub!(:refresh_token) 
              @author.stub!(:forget_me)
              @author.stub!(:remember_token).and_return(token_value) 
              @author.stub!(:remember_token_expires_at).and_return(token_expiry)
              @author.stub!(:remember_token?).and_return(has_request_token == :valid)
              if want_remember_me
            ...
            if (has_request_token == :valid)
              it 'does not make new token'   do @author.should_not_receive(:remember_me);   do_create end
              it 'does refresh token'        do @author.should_receive(:refresh_token);     do_create end 
              it "sets an auth cookie"       do do_create;  end
            else
              if want_remember_me
                it 'makes a new token'       do @author.should_receive(:remember_me);       do_create end 
                it "does not refresh token"  do @author.should_not_receive(:refresh_token); do_create end
                it "sets an auth cookie"       do do_create;  end
              else 
                it 'does not make new token' do @author.should_not_receive(:remember_me);   do_create end
                it 'does not refresh token'  do @author.should_not_receive(:refresh_token); do_create end 
                it 'kills user token'        do @author.should_receive(:forget_me);         do_create end 
              end
            end
          end # inner describe
        end
  ...
  describe "on failed login" do
    before do
      Author.should_receive(:authenticate).with(anything(), anything()).and_return(nil)
      login_as :quentin
    end
